...
*/
   ...
   // If mode=0 use for arGetTransMat instead of arGetTransMatCont
   mode=1;
}
...
void 
ARSinglePublisher::camInfoCallback(const sensor_msgs::CameraInfoConstPtr& cam_info){
	if(!getCamInfo_){
		cam_info_=(*cam_info);
...
ARSinglePublisher::arInit ()
{
   ...
   sz = cvSize(xsize_,ysize_);
   capture=cvCreateImage(sz, IPL_DEPTH_8U, 4);
}

void
ARSinglePublisher::getTransformationCallback (const sensor_msgs::ImageConstPtr& image_msg)
{
   ARUint8 *dataPtr;
   ...
   * ARToolKit configure option (see config.h).*/
   try
   {
      capture=bridge_.imgMsgToCv(image_msg, "bgr8");
   }
   catch (sensor_msgs::CvBridgeException& e)
   {
      ROS_ERROR("Could not convert from '%s' to 'bgr8'.", image_msg->encoding.c_str());
   }
   //cvConvertImage(capture,capture,CV_CVTIMG_FLIP); //flip image
   dataPtr = (ARUint8*)capture->imageData;
   ...
   {		
      ...
      ROS_INFO(" QUAT: Pos x: %3.1f  y: %3.1f  z: %3.1f", pos[0], pos[1], pos[2]);
      ROS_INFO("     Quat qx: %3.2f qy: %3.2f qz: %3.2f qw: %3.2f", quat[0], quat[1], quat[2], quat[3]);
      ...
      ROS_INFO ("ar_single tf published");
   }
   else 
   {
